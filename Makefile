...
# Linux
APPIMAGE_NAME = images_load.AppImage
APPIMAGE_DIR = $(RELEASE_DIR)/AppDir
APPIMAGE_DESKTOP_FILE = $(RESOURCES_DIR)/linux/images_load.desktop
APPIMAGE_ICON_FILE = $(RESOURCES_DIR)/icons/256x256/logo.png

TAR_NAME = images_load.tar.gz
...
ifdef OPENGL
  DMG_NAME :=images_load-opengl.dmg
  APPIMAGE_NAME :=images_load-opengl.AppImage
else
  DMG_NAME :=images_load.dmg
  APPIMAGE_NAME :=images_load.AppImage
  FEATURE_FLAG :=
endif
